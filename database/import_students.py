    ...
    for entry in data:
        ...
        for year_block in academic.get("gradeRecordList", []):
            for rec in year_block.get("GradeRecords", []):
                ...
                # Delete existing record before re-insert to avoid duplicates on re-run
                cur.execute(
                    "DELETE FROM course_record WHERE student_id = %s AND course_code = %s",
                    (student_id, course_code),
                )
                cur.execute(
                    """
                    INSERT INTO course_record (student_id, course_code, score, course_status)
                    VALUES (%s, %s, %s, %s)
                    """,
                    (student_id, course_code, score_val, status),
                )
...
def main():
    default_path = os.path.join(os.path.dirname(__file__), "exportStudentData.json")
